...
export default function JobMonitoring() {
  const { user } = useAuth();
  const [jobs, setJobs] = useState<WooCommerceJob[]>([]);
      ...
      <div className="bg-white shadow overflow-hidden sm:rounded-md">
        <div className="px-4 py-5 sm:px-6">
          <div className="flex justify-between items-center">
            <div>
              <h3 className="text-lg leading-6 font-medium text-gray-900">Sync Jobs</h3>
              <p className="mt-1 max-w-2xl text-sm text-gray-500">
                Monitor the status of your WooCommerce data synchronization jobs
              </p>
            </div>
            <button
              onClick={fetchJobs}
        ...
        {jobs.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Jobs will appear here once you start syncing your WooCommerce clients.
            </p>
          </div>
        ) : (
                  ...
                  <tr key={job.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      <div className="flex items-center space-x-2">
                        <span>Processed: {job.orders_processed}</span>
                        <span>•</span>
                        <span>Created: {job.orders_created}</span>
                        <span>•</span>
                        <span>Updated: {job.orders_updated}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
