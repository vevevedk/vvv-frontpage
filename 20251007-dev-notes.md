...
### **1. Nginx Configuration Reset**
**File**: `/etc/nginx/sites-available/veveve.dk`
...
server {
    ...
    # API routes to invest backend (port 8002)
    location /api/ {
        proxy_pass http://127.0.0.1:8002;
        proxy_set_header Host invest.veveve.dk;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    }
    
    # Admin routes to invest backend
    location /admin/ {
        proxy_pass http://127.0.0.1:8002;
        proxy_set_header Host invest.veveve.dk;
        proxy_set_header X-Real-IP $remote_addr;
    ...
    # Frontend routes to vvv-frontpage (port 3000)
    location / {
...
### **2. Backend ALLOWED_HOSTS Configuration**
**File**: `backend/.env` (Docker Compose environment)

**Updated Configuration**:
```bash
ALLOWED_HOSTS=localhost,127.0.0.1,veveve.dk,www.veveve.dk,invest.veveve.dk,www.invest.veveve.dk,209.38.98.109
CSRF_TRUSTED_ORIGINS=https://veveve.dk,https://www.veveve.dk,https://invest.veveve.dk,https://www.invest.veveve.dk
```
...
**Scripts Created**:
- `scripts/fix-backend-allowed-hosts.sh` - Automated ALLOWED_HOSTS updates
- `scripts/post-deploy-verify.sh` - Health checks across ports and vhosts
- `DEPLOY_README.md` - Standardized multi-app deployment guide
...
curl -sS -i https://veveve.dk/api/auth/login/ \
  ...
  --data '{"email":"andreas@veveve.dk","password":"avxzVvv2k25!!"}' -k
