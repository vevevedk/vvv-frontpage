...
# Fix 502 Bad Gateway errors for veveve.dk
...
# Step 2: Stop any existing containers that might be conflicting
print_status "Stopping existing containers..."
cd "$ROOT_DIR"
docker-compose down || true

# Step 3: Clean up any orphaned containers and networks
...
if lsof -i :8001 > /dev/null 2>&1; then
    print_warning "Port 8001 is in use. Killing processes..."
    sudo pkill -f ":8001" || true
fi

# Step 5: Ensure nginx configuration is correct
print_status "Installing correct nginx configuration..."
...
if curl -f -s --max-time 10 https://veveve.dk/health > /dev/null; then
    print_status "✅ veveve.dk is responding!"
else
    print_warning "⚠️ veveve.dk is not responding yet"
    print_info "This might take a few more seconds to propagate"
fi

# Step 14: Show final status
echo ""
echo "🎉 502 Error Fix Completed!"
echo "=========================="
echo ""
...
if curl -f -s --max-time 10 https://veveve.dk > /dev/null; then
    print_status "🌐 veveve.dk is now accessible!"
else
    print_warning "🌐 veveve.dk may need a few more minutes to become accessible"
    print_info "Try: curl -I https://veveve.dk"
fi
...
echo "3. Test the site: https://veveve.dk"
echo ""
