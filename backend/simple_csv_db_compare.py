#!/usr/bin/env python
"""
Simple comparison of WooCommerce CSV data against production database
Direct database connection without Django setup
"""
import csv
import json
import psycopg2
...
def connect_to_db():
    ...
    # Using the production database credentials from env/backend.prod.env
    conn = psycopg2.connect(
        host="localhost",  # Assuming local connection or tunnel
        port="5432",
        database="vvv_database",
        user="vvv_user",
        password="CHANGE-THIS-TO-A-SECURE-PASSWORD"  # This needs to be updated with real password
    )
    return conn
...
def main():
    ...
    try:
        ...
        print("\nTo connect to the production database, you need to:")
        print("1. SSH tunnel to the production server")
        print("2. Update the database credentials in this script")
        print("3. Or provide the correct connection details")
        return
    
    try:
        # Query database metrics
        db_metrics = query_db_metrics(conn)
